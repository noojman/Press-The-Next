...
public class LetterBoardBehavior : MonoBehaviour {
	...
	void Update () {
		...
		if (startAnimation == true && stage == 2) {
			...
			if (LetterGameController.highCounter > 53) {
				stage = 0;
				change = false;
			} else {
				...
				} else if (currentSpriteNum == 51) {
					GetComponent<SpriteRenderer> ().sprite = s51;
				} else if (currentSpriteNum == 52) {
					GetComponent<SpriteRenderer> ().sprite = s52;
				}
				startAnimation = true;
				stage++;
			}
	...
	void Set (int lowNum) {
		...
		if (lowNum == (currentSpriteNum)) {
			pop.Play ();
			if (currentSpriteNum == 1) {
				LetterTimerBehavior.start = true;
			}
			Debug.Log ("Set change to true");
			currentSpriteNum += 25;
			LetterLabelBehavior.change = true;
			change = true;
		} else {
